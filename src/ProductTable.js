...
class ProductTable extends React.Component {
    ...
    render() {
        ...
        this.sortProducts().forEach((product) => {
            if (product.name.indexOf(this.props.filterText) === -1 || ((product.stocked == false) && (this.props.inStockOnly))) {
                return;
            }
            rows.push(<ProductRow product={product} key={product.id} onDestroy=
                {this.handleDestroy}></ProductRow>);
        });
